...
class DuelDQNAgent(Agent):
    def __init__(self, number_actions, env, batch_size):
        ...
        self.local_network = DuelDQN(number_actions, env, batch_size)
    ...
    def step(self, state):
        ...
        action = self.local_network(state).max(0)[1].data[0][0] #[0] is value, [1] is the index for the first []
    ...
    def train(self):
        ...
        for memory_state, memory_reward, memory_action, memory_next_state, memory_done in memories:
            ...
            q_value = self.local_network(memory_state).gather(0, memory_action)

            target = self.local_network(memory_next_state).max(0)[0] * 0.90 + memory_reward
...
class DuelDQN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        qValue = value + advantage - advantage.mean(1).expand_as(advantage)
